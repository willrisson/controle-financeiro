...
# --- CONFIGURAÇÃO ---
st.set_page_config(
    page_title="Controle Financeiro Familiar", page_icon="💳", layout="centered"
)
...
def classificar_categoria_groq(
    descricao, status_container, categorias_disponiveis
):
  ...
  try:
    ...
    if "choices" in data and len(data["choices"]) > 0:
      cat = json.loads(data["choices"][0]["message"].get("content", "{}")).get(
          "categoria", "Outros"
      )
...
# ==========================================
aba_lancamento, aba_graficos = st.tabs(
    ["📝 Lançar Gasto", "📊 Gráficos por Mês"]
)

# ==========================================
# ABA 1: LANÇAMENTO DE GASTOS
# ==========================================
with aba_lancamento:
  ...
  lista_gastadores_atualizada = sorted(
      list(set(LISTA_GASTADORES_BASE + st.session_state["gastadores_extras"]))
  )
  ...
  if quem_gastou_selecionado == "Outro...":
    ...
    with col_q_txt:
      st.markdown(
          "<div style='padding-top: 6px; color: gray; font-size: 14px;'>Membro"
          " cadastrado selecionado</div>",
  ...
  descricao_gasto = st.text_input(
      "Descrição do Gasto",
      placeholder="Ex: Meta Quest 3s",
      key="input_descricao",
  )
  valor_texto = st.text_input(
      "Valor Total (R$)",
      placeholder="Ex: 50, 50,00 ou 1800,00",
      key="input_valor",
  )
  ...
  lista_categorias_atualizada = sorted(
      list(set(lista_base_carregada + st.session_state["categorias_extras"]))
  )

  categoria_sugerida = "Outros"
  if descricao_gasto and descricao_gasto.strip():
    categoria_sugerida = classificar_categoria_groq(
        descricao_gasto, None, lista_categorias_atualizada
    )
  ...
  with col_parc_num:
    num_parcelas_str = st.text_input(
        "Número de Parcelas",
        disabled=not parcelado,
        placeholder="Ex: 10",
        key="input_num_parcelas",
    )
  ...
  if btn_enviar:
    if not quem_gastou or quem_gastou == "Outro...":
      st.warning(
          "Por favor, selecione ou digite o nome de quem está gastando."
      )
    elif (
        not descricao_gasto
        or not descricao_gasto.strip()
        or not valor_texto
        or not valor_texto.strip()
    ):
  ...
  if st.session_state["processando_envio"]:
    try:
      ...
      st.error(
          "Formato de valor inválido! Use números como 50 ou 1800,00"
      )
      valor_gasto = 0.0
      st.session_state["processando_envio"] = False
    ...
    try:
      num_parcelas = (
          int(num_parcelas_str)
          if parcelado and num_parcelas_str.strip()
          else 1
      )
      if num_parcelas < 1:
        num_parcelas = 1
    ...
    if valor_gasto > 0:
      with st.status("Processando lançamento...", expanded=True) as status:
        ...
        status.write(
            f"📂 Conectando à planilha e buscando aba '{NOME_ABA}'..."
        )
        try:
          sh = client.open_by_key(SPREADSHEET_ID)
          worksheet = sh.worksheet(NOME_ABA)
          ...
          status.update(
              label="❌ Erro ao localizar a planilha/aba!", state="error"
          )
          st.error(f"Detalhe: {e}")
          st.session_state["processando_envio"] = False
          st.stop()
        ...
        cabecalho_final = cabecalho_base + [
            m for m in meses_existentes if m not in cabecalho_base
        ]

        data_hoje = datetime.now() - timedelta(hours=3)
        detalhe_pagamento = f"{forma_pagamento} ({banco_emissor})"
        valor_parcela = (
            round(valor_gasto / num_parcelas, 2)
        ...
        for i in range(num_parcelas):
          ...
          data_vencimento = datetime(
              ano_alvo, mes_alvo, min(data_hoje.day, 28)
          )
          nome_mes_coluna = obter_nome_mes_ano(data_vencimento)

          parcelas_info.append(
              {"mes_ano": nome_mes_coluna, "valor": valor_parcela}
          )
        ...
        linha_dados[cabecalho_final.index("Data/Hora")] = (
            f"'{data_hoje.strftime('%d/%m/%Y %H:%M:%S')}"
        )
        linha_dados[cabecalho_final.index("Quem Gastou")] = quem_gastou
        linha_dados[cabecalho_final.index("Descrição")] = (
            descricao_gasto
            if not parcelado
            else f"{descricao_gasto} ({num_parcelas}x)"
        )
        linha_dados[cabecalho_final.index("Categoria")] = categoria_final
        linha_dados[cabecalho_final.index("Forma de Pagamento")] = (
            detalhe_pagamento
        )
        ...
        worksheet.update(
            intervalo, [linha_dados], value_input_option="USER_ENTERED"
        )

        status.update(
            label="🎉 Gasto registrado com sucesso!",
            state="complete",
            expanded=False,
        )
        if parcelado:
          st.success(
              f"✅ Gasto de R$ {valor_gasto:.2f} por **{quem_gastou}** na"
              f" categoria **{categoria_final}** parcelado em"
              f" {num_parcelas}x!"
          )
        else:
          st.success(
              f"✅ Gasto de R$ {valor_gasto:.2f} por **{quem_gastou}**"
              f" registrado na categoria **{categoria_final}**!"
          )
...
# ==========================================
with aba_graficos:
  st.title("📊 Relatório de Gastos por Mês")
  ...
  try:
    ...
    if not dados:
      st.info(
          "Ainda não há dados cadastrados na planilha para gerar gráficos."
      )
    else:
      import pandas as pd
      ...
      # Identificar colunas de meses (colunas a partir da coluna G em diante)
      colunas_meses = [
          col for col in df.columns if "/" in col and col != "Data/Hora"
      ]

      if not colunas_meses:
        st.info("Nenhuma coluna de mês identificada ainda.")
      else:
        # Seletor de Mês
        mes_selecionado = st.selectbox(
            "Selecione o Mês para Análise:", colunas_meses
        )

        # Converter valores da coluna do mês para numérico com segurança
        df[mes_selecionado] = pd.to_numeric(
            df[mes_selecionado]
            .astype(str)
        ...
        df_agrupado = (
            df.groupby("Categoria")[mes_selecionado].sum().reset_index()
        )
        df_agrupado = df_agrupado[
            df_agrupado[mes_selecionado] > 0
        ]  # Remove categorias zeradas

        if df_agrupado.empty:
          st.warning(f"Não há gastos registrados para {mes_selecionado}.")
        else:
          total_mes = df_agrupado[mes_selecionado].sum()

          # Exibir o Total Geral em destaque no topo
          st.metric(
              label=f"💰 Total Gasto em {mes_selecionado}",
              value=f"R$ {total_mes:,.2f}".replace(",", "X")
              .replace(".", ",")
              .replace("X", "."),
          )

          # Criar Gráfico de Pizza com valores dentro de cada fatia
          fig = px.pie(
              df_agrupado,
              names="Categoria",
              values=mes_selecionado,
              title=(
                  f"Distribuição de Gastos por Categoria - {mes_selecionado}"
              ),
              hole=0.3,  # Estilo rosca elegante
          )
          fig.update_traces(
              textinfo="label+value", textfont_size=14
          )  # Mostra o nome da categoria e o valor na fatia

          st.plotly_chart(fig, use_container_width=True)

  except Exception as e:
    st.error(f"Erro ao carregar dados para os gráficos: {e}")
